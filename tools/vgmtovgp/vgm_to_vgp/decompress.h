...
streampos fileSize_wide(const wchar_t* filePath)
{
	streampos fsize = 0;
	ifstream file(filePath, std::ios::binary);
	fsize = file.tellg();
...
inline static bool Decompress(const char* pSrc_filename, const char* pDst_filename)
{
	...
	infile_size = fileSize(pSrc_filename);
	pInfile.open(pSrc_filename, ios::in | ios::binary);




	pOutfile.open(pDst_filename, ios::binary);

	//pInfile.seekSet(10);
	pInfile.seekg(10, ios::beg);
	...
	for (; ; )
	{
		...
		if ((status <= TINFL_STATUS_DONE) || (!avail_out)) //This section of code isn't even called as 'status' is always in it's error state (-1)
		{
			...
			pOutfile.write((char*)s_outbuf, n);
